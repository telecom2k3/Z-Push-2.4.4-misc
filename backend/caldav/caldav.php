...
class BackendCalDAV extends BackendDiff {
    ...
    public function Logon($username, $domain, $password) {
		...
		if (defined(W10_FIX)) {
			$w10fix = sprintf("%s", W10_FIX);
		} else {
			$w10fix = "false";
		}
		if (defined(HTML_DETOX)) {
			$htdetox = sprintf("%s", HTML_DETOX);
		} else {
			$htdetox = "false";
		}
    ...
    private function _ParseASEventToVEvent($data, $id) {
        ...
        if (isset($data->starttime)) {
            if ($data->alldayevent == 1) {
				...
				if ($w10fix == "true") {
					$vevent->AddProperty("DTSTART", $this->_GetDateFromUTC("Ymd", $data->endtime, $data->timezone), array("VALUE" => "DATE"));
				} else {	
        ...
        if (isset($data->rtf)) {
            ...
            $rtfparser->parse();
			if ($htdetox == "true") {
				$rtfoutput = "$rtfparser->out";
				$rtfoutput = strip_tags($rtfoutput);
				$rtfoutput = str_replace("&nbsp;"," ", $rtfoutput);
				$rtfoutput = str_replace("\n","\r\n", $rtfoutput);
				$vevent->AddProperty("DESCRIPTION", $rtfoutput);
			} else {
        ...
        if (isset($data->asbody->data)) {
            ...
            if (strlen($asbody) > 0) {
				$asbody = strip_tags($asbody);
				$asbody = str_replace("&nbsp;"," ", $asbody);
				$asbody = str_replace("\n","\r\n", $asbody);
        ...
        if (isset($data->asbody->data)) {
            if (isset($data->nativebodytype) && $data->nativebodytype == SYNC_BODYPREFERENCE_RTF) {
				...
				if ($htdetox == "true") {
					...
					$rtfoutput = str_replace("&nbsp;"," ", $rtfoutput);
					$rtfoutput = str_replace("\n","\r\n", $rtfoutput);
					$vtodo->AddProperty("DESCRIPTION", $rtfoutput);
				} else {
